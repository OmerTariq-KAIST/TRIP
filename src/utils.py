...
class ProposedTransform_v2:
    def __init__(self, max_stretch):
        self.max_stretch = max_stretch

    def __call__(self, feat, targ, **kwargs):
        time_steps = feat.shape[0]
        stretch_factor = 1 + np.random.uniform(-self.max_stretch, self.max_stretch)
        ...
        return feat_aug, targ



class ProposedTransform:
    def __init__(self, max_angle, max_skew, max_stretch):
    ...
    def __call__(self, feat, targ, **kwargs):
        ...
        skew = np.random.uniform(-self.max_skew, self.max_skew)
        stretch = 1 + np.random.uniform(-self.max_stretch, self.max_stretch)
        ...
        # Skew matrix
        skew_matrix = np.array([[1, skew],
                                 [0, 1]])
        
        # Combine transformations
        transform = rm @ skew_matrix
        transform *= stretch  # Apply uniform scaling
        
        feat_aug = np.copy(feat)
        targ_aug = np.copy(targ)
        
        feat_aug[:, :2] = np.matmul(transform, feat[:, :2].T).T
        feat_aug[:, 3:5] = np.matmul(transform, feat[:, 3:5].T).T
        targ_aug[:2] = np.matmul(transform, targ[:2].T).T

        return feat_aug, targ_aug


class ProposedTransform_v1:
    def __init__(self, max_angle, max_scale, max_skew, noise_level):
        self.max_angle = max_angle
    ...
    def __call__(self, feat, targ, **kwargs):
        ...
        # Combine transformations
        transform = scale * (rotation_matrix @ skew_matrix)
        ...
        feat_aug += np.random.normal(0, self.noise_level, feat.shape)
