...
def compute_drift(est, gt):
    """
    Computes D_drift as a percentage, the absolute difference between the estimated 
    and ground-truth trajectory lengths divided by the ground-truth trajectory length.

    Args:
        est: estimated trajectory (numpy array with shape (N, 3) for 3D or (N, 2) for 2D).
        gt: ground truth trajectory with the same shape as est.

    Returns:
        D_drift: The drift percentage.
    """
    # Calculate the length of the estimated trajectory
    est_length = np.sum(np.linalg.norm(np.diff(est, axis=0), axis=1))
    # Calculate the length of the ground-truth trajectory
    gt_length = np.sum(np.linalg.norm(np.diff(gt, axis=0), axis=1))
    # Calculate D_drift as a percentage
    d_drift = (np.abs(est_length - gt_length) / gt_length) * 100
    return d_drift
